...
local instrThreshold = 0x02073C74 -- EUR (USA: 0x02073C64)
...
-- coolup
local threshold1 = 1000
local threshold2 = 0
local thresholdMax = 10000

local ATable = {}
local ALookup = {}
local inputs = {}

local prevTrack = false
local toggleVis = true
local prevVis = false
...
local baseX = 0
local baseY = 0-- 173 bottom of top, 192 top of bottom, 360 bottom of bottom
...
local function handleToggleVis(inputs)
   local pressed = inputs.L and inputs.R and inputs.A
   if pressed and not prevVis then
      toggleVis = not toggleVis
   end
   prevVis = pressed
end
...
local function main()
--memory.writedword(addrAT, 0xFFFFFFFF) -- AT
--memory.writedword(addrCoolup, 0x0000FFFF) -- coolup
--memory.writedword(0x027E38B0, 0x0002f000) -- field camera tilt
--memory.writedword(0x027E38B4, 0x00030000) -- field camera zoom
   ...
   local trackPressed = inputs.start and inputs.A
   local initTracker = trackPressed and not prevTrack
   prevTrack = trackPressed

   handleToggleVis(inputs)
   ...
   if toggleVis then
      ...
      if coolup < threshold1 then
         coolupColour = "white"
      elseif coolup < threshold2 then
         coolupColour = "yellow"
      elseif coolup < thresholdMax then
         coolupColour = "green"
      else
         coolupColour = "green"
         coolup = "10000+"
...
memory.registerexec(instrThreshold, function()
   threshold2 = memory.getregister("r0")
end)
