...
local function handleToggleVis(inputs)
   local pressed = inputs.L and inputs.R and inputs. B
...
local function makeKey(hi, lo)
   return string.format("%08X%08X", hi, lo)
end

function main()
-- Poke addresses
--memory.writedword(addr.en_hi, 0xFAD00456)
--memory.writedword(addr.en_lo, 0xB7841DBD)
--memory.writedword(0x02399190, 0x01010100) -- offsets for the opening party animation
   ...
   local trackPressed = inputs.start and inputs.B
   ...
   if initTracker then
      ...
      for i = 1, maxAdvances do
         local next_hi = bt_rand()
         local next_lo = lowbyte

         local k = makeKey(next_hi, next_lo)
         BTableFull[i] = {next_hi, next_lo}
         BLookup[k] = i
      end
