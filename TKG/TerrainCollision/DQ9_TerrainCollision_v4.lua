...
local function readHeight(base)
    return fx32(memory.readdword(base + 0x68))
end
...
local function drawWorldCylinder(cx, cy, cz, r, h, cam, proj, colour)
    local segments = 32
    local step = (math.pi * 2) / segments

    local minX, maxX = math.huge, -math.huge
    local minBX, minBY, minBZ
    local maxBX, maxBY, maxBZ
    ...
    for i = 0, segments do
        ...
        local rx, ry, rz = matMul3x4(bx, by, bz, cam)

        if rx < minX then
            minX = rx
            minBX, minBY, minBZ = bx, by, bz
        end

        if rx > maxX then
            maxX = rx
            maxBX, maxBY, maxBZ = bx, by, bz
        end
    end

    -- draw top & bottom rings (optional, keep your existing circle calls if you want)
    drawWorldCircle(cx, cy, cz, r, cam, proj, colour)
    drawWorldCircle(cx, cy + h, cz, r, cam, proj, colour)

    -- draw ONLY 2 vertical lines
    if minBX then
        drawWorldLine(minBX, minBY, minBZ, minBX, minBY + h, minBZ, cam, proj, colour)
    end

    if maxBX then
        drawWorldLine(maxBX, maxBY, maxBZ, maxBX, maxBY + h, maxBZ, cam, proj, colour)
    end
end
...
local function getUpWarp()
    local prevY = memory.readdwordsigned(addr.mc + 0x48)
    local maxY = prevY + pHeight
    local newY = ((prevY - 0xA000) - maxY) * mult + 0x800
    newY = bit.bor(bit.rshift(newY, 12), 0xFFF00000)
    newY = newY + maxY + 0x199
    return newY
end
...
memory.registerexec(0x020320FC, function()
    mult = memory.getregister("r1")
end)


local function main()
    --memory.writeword(0x020FDD4C, 0) -- coolup
    --memory.writedword(addr.mc + 0x48, 0x7fc) -- player Y
    --memory.writeword(addr.mc + 0x68, 0x1800) -- player hitbox height
    ...
    if inBattle == 0 then
        ...
        local ph = readHeight(addr.mc)
        drawWorldCylinder(px, py, pz, pr, ph, cam, proj, "cyan")

        upWarp = getUpWarp()
        upWarph = getHex(upWarp, 8)
        upWarpfx = string.format("%8.3f", upWarp / scale)
        gui.text(5, 5, "UPWARP: " .. upWarph .. " (" .. upWarpfx .. ")")
